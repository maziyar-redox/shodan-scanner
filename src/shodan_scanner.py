...
if __name__ == "__main__":
	...
	argparser.add_argument("-s", "--search", type=str, help="search query", default="tornado", action="store", required=True)
	argparser.add_argument("-o", "--output", type=str, help="output filename", default="out.txt", action="store", required=True)
	argparser.add_argument("-p", "--pages", type=str, help="Page size", default="1", action="store", required=True)
	...
	try:
		...
		if perform_query.upper() == "Y":
			...
			for x in range(0, int(args.pages)):
				print("[+] Info: Fetching page {0}".format(x + 1))
